...
# Genel bilgilerin elde edilmesi
...
def remove_highly_correlated_features(df, threshold, plot=False):
    """
    Verilen veri çerçevesindeki yüksek korelasyona sahip değişkenleri siler.

    Parametreler:
    df (pandas.DataFrame): İşlem yapılacak veri çerçevesi
    threshold (float): korelasyon eşiği

    Döndürdüğü Değer:
    pandas.DataFrame: korelasyonu yüksek değişkenlerin çıkarıldığı veri çerçevesi
    """
    # Korelasyon matrisini hesapla
    corr_matrix = df.corr().abs()

    # Üst üçgeni al ve korelasyon matrisindeki tüm sütunlar için yüksek korelasyona sahip sütunları al
    upper = corr_matrix.where(np.triu(np.ones(corr_matrix.shape), k=1).astype(np.dtype(bool) ))
    to_drop = [column for column in upper.columns if any(upper[column] > threshold)]

    # Yüksek korelasyona sahip sütunları çıkar
...
df1 = remove_highly_correlated_features(df,0.9,plot=True)
#FEATURE ENGİNEERİNG
...
for col in num_cols:
    outlier_thresholds(df, col, low_quantile=0.10, up_quantile=0.90)
...
df.drop(labels ='NewLeague', axis=1, inplace=True)
df.drop(labels ='Walks', axis=1, inplace=True)
...
#  Train ve Test verisini ayırınız. (Salary değişkeni boş olan değerler test verisidir.)
train_df = df[df['Salary'].notnull()]
test_df = df[df['Salary'].isnull()]
y = train_df(np.log1p(df['Salary']))
...
model = LGBMRegressor()
model.fit(X, y)
predictions = model.predict(test_df.drop(["Salary"], axis=1))

dictionary = {"Salary":predictions}
dfSubmission = pd.DataFrame(dictionary)
dfSubmission.to_csv("housePricePredictions.csv", index=False)

xs=pd.read_csv("housePricePredictions.csv")